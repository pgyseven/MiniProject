...
public class HBoardServiceImpl implements HBoardService {
   ...
   public boolean saveBoard(HBoardDTO newBoard) throws Exception {
      ...
      if (bDao.insertNewBoard(newBoard) == 1) {
         ...
         System.out.println("방금 저장된 글 번호 확인 !!!!!!!!!!!!: " + newBoardNo);
         ...
         for (BoardUpFilesVODTO file : newBoard.getFileList()) { // newBoard.getFileList() 사이즈가 0이면 즉 없으면 첨부된게 포문이 안도니깐 이프문 필요 없음
            System.out.println("첨부파일 저장하러 갑니다");
   ...
   public Map<String, Object> read(int boardNo) throws Exception { //Map<String, Object> 여기서 오브젝트는 HBoardVO ,<BoardUpFilesVODTO> 이걸 합쳐서 보내야하는데 이들은 부모가 없다 그러나 자바 모두의 부모인 오브젝트를 쓰면 된다.
      //게시글 조회를 한다... select(첨부파일이 있다면 첨부파일도 조회한다. select)
      System.out.println(bDao.selectBoardByBoardNo(boardNo).toString());
     
      HBoardVO board = bDao.selectBoardByBoardNo(boardNo);
      
     List<BoardUpFilesVODTO> fileList = bDao.selectBoardUpFilesByBoardNo(boardNo);
     // 여기까지 둘다 샐랙트라 롤백이 필요없어서 아래가 완성 안되었을때는  @Transactional(rollbackFor = Exception.class) 이렇게만 테스트 한다.
     
     Map<String, Object> resultMap = new HashMap<String, Object>();
     
     //upcasting : 자식 객체의 데이터 타입이 부모객체의 데이터 타입으로 변화 (자식객체의 특성이 사라진다)
     resultMap.put("board", board); // board객체( HBoardVO  타입) 가 map 에 담길때 object 타입으로 업캐스팅 된다. upcasting/ 클래스는 사용자가 만든 데이터 타입 int String  이런건 자바가 만든것 이런걸 묶어서 클래스라는 내가 만든 객체가 된다.
     resultMap.put("fileList", fileList); //fileList 객체가 (BoardUpFilesVODTO  타입) 가 map 에 담길때 object 타입으로 업캐스팅 된다. / 반대 의미로 다운캐스팅도 있다.
     
     
     
     return resultMap; //얘는 데이터 타입이 이제 한가지니깐 얘로 반환한다.
